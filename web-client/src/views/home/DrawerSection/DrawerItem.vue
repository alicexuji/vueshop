<template>
  <div class="pro_item">
    <h4 class="item_title"><span>汪星人商城自营店</span><span class="item_one">{{ goods.sellPrice }}</span></h4>
    <div class="item_content">
      ...
      <div class="item-info">{{ goods.name }}</div>
      <div class="item-amout">
        <el-input-number size="mini" v-model="goods.sale" :min="0" :max="goods.sale"
                         @change="updateGoodsCount(goods, goods.sale)"></el-input-number>
      </div>
      <div class="item-sum"><strong>{{ goods.sale * goods.sellPrice }}</strong></div>
    </div>
  </div>
</template>
